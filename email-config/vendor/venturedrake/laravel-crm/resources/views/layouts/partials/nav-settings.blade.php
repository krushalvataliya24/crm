<ul class="nav nav-pills flex-column" role="tablist">
    ...
    @endcan
    @can('view crm email config')
        <li class="nav-item">
            <a class="nav-link {{ (strpos(Route::currentRouteName(), 'laravel-crm.email-config') === 0) ? 'active' : '' }}" href="{{ url(route('laravel-crm.email-config.index')) }}" role="tab" aria-controls="labels" aria-selected="false">{{ ucwords(__('laravel-crm::lang.email_config')) }}</a>
        </li>
    @endcan
    @can('view crm labels')
        <li class="nav-item">
